...
var require_range = __commonJS((exports2, module2) => {
  var Range10 = class {
    constructor(range, options) {
      options = parseOptions(options);
      if (range instanceof Range10) {
        if (range.loose === !!options.loose && range.includePrerelease === !!options.includePrerelease) {
          return range;
        } else {
          return new Range10(range.raw, options);
        }
      }
...
var require_named_js_regexp = __commonJS((exports2, module2) => {
  ...
  (function(window9) {
    ...
    if (typeof exports2 !== "undefined") {
      ...
      window9.createNamedRegex = createNamedRegex;
    }
...
var PythonFormattingEditProvider = class {
  constructor() {
    ...
    this.disposables = [];
    this.pythonSettings = PythonSettings.getInstance();
    this.outputChannel = import_coc7.window.createOutputChannel("coc-pyright-formatting");
...
async function generateIsortFixDiff(extensionRoot, uri, token) {
...
async function sortImports(extensionRoot, outputChannel) {
  ...
  const uri = import_coc8.Uri.parse(doc.uri);
  if (doc.lineCount <= 1) {
    return;
  }
  try {
...
${message}`);
    import_coc8.window.showMessage(`Failed to format import by isort`, "error");
  }
}
...
var BaseLinter = class {
  ...
  async run(args, document, cancellation, regEx = REGEX) {
    if (!this.info.isEnabled(import_coc9.Uri.parse(document.uri))) {
      return [];
    }
    const executionInfo = this.info.getExecutionInfo(args, import_coc9.Uri.parse(document.uri));
    try {
      const pythonToolsExecutionService = new PythonExecutionService();
      const result = await pythonToolsExecutionService.exec(executionInfo, {cwd: import_coc9.workspace.root, token: cancellation, mergeStdOutErr: false});
...
`);
      ...
      await this.errorHandler.handleError(error, import_coc9.Uri.parse(document.uri), executionInfo).catch(() => {
      });
      return [];
    }
  }
...
var Bandit = class extends BaseLinter {
  ...
  async runLinter(document, cancellation) {
    const messages = await this.run(["-f", "custom", "--msg-template", "{line},0,{severity},{test_id}:{msg}", "-n", "-1", import_coc10.Uri.parse(document.uri).fsPath], document, cancellation);
...
var Flake8 = class extends BaseLinter {
  ...
  async runLinter(document, cancellation) {
    const messages = await this.run(["--format=%(row)d,%(col)d,%(code).1s,%(code)s:%(text)s", import_coc11.Uri.parse(document.uri).fsPath], document, cancellation);
    messages.forEach((msg) => {
...
var MyPy = class extends BaseLinter {
  ...
  async runLinter(document, cancellation) {
    const messages = await this.run([import_coc12.Uri.parse(document.uri).fsPath], document, cancellation, REGEX2);
    messages.forEach((msg) => {
...
var Pep8 = class extends BaseLinter {
  ...
  async runLinter(document, cancellation) {
    const messages = await this.run(["--format=%(row)d,%(col)d,%(code).1s,%(code)s:%(text)s", import_coc13.Uri.parse(document.uri).fsPath], document, cancellation);
    messages.forEach((msg) => {
...
var PyDocStyle = class extends BaseLinter {
  ...
  async runLinter(document, cancellation) {
    const messages = await this.run([import_coc15.Uri.parse(document.uri).fsPath], document, cancellation);
    messages.forEach((msg) => {
  ...
  async parseMessages(output, document, _token, _regEx) {
    ...
    const baseFileName = path6.basename(import_coc15.Uri.parse(document.uri).fsPath);
...
var PyLama = class extends BaseLinter {
  ...
  async runLinter(document, cancellation) {
    const messages = await this.run(["--format=parsable", import_coc16.Uri.parse(document.uri).fsPath], document, cancellation, REGEX3);
    messages.forEach((msg) => {
      msg.severity = LintMessageSeverity.Warning;
    });
    return messages;
...
var Pylint = class extends BaseLinter {
  ...
  async runLinter(document, cancellation) {
    let minArgs = [];
    const uri = import_coc17.Uri.parse(document.uri);
    const settings = this.pythonSettings;
    if (settings.linting.pylintUseMinimalCheckers && this.info.linterArgs(uri).length === 0 && !await this.hasConfigrationFileInWorkspace(path7.dirname(uri.fsPath), import_coc17.workspace.root) && !await this.hasConfigurationFile(import_coc17.workspace.root)) {
      minArgs = [
        "--disable=all",
...
var Pytype = class extends BaseLinter {
  ...
  async runLinter(document, cancellation) {
    const args = [];
    if (await this.hasConfigurationFile(import_coc18.workspace.root)) {
      args.push(...["--config", pytypecfg]);
    }
    args.push(import_coc18.Uri.parse(document.uri).fsPath);
  ...
  async parseMessages(output, document, token, regEx) {
    ...
    const messages = (await super.parseMessages(newOutput, document, token, regEx)).filter((msg) => {
      return msg.file && msg.file === import_coc18.Uri.parse(document.uri).fsPath;
    });
...
lintSeverityToVSSeverity.set(LintMessageSeverity.Error, import_coc19.DiagnosticSeverity.Error);
lintSeverityToVSSeverity.set(LintMessageSeverity.Hint, import_coc19.DiagnosticSeverity.Hint);
lintSeverityToVSSeverity.set(LintMessageSeverity.Information, import_coc19.DiagnosticSeverity.Information);
lintSeverityToVSSeverity.set(LintMessageSeverity.Warning, import_coc19.DiagnosticSeverity.Warning);
...
var LintingEngine = class {
  constructor() {
    ...
    this.outputChannel = import_coc19.window.createOutputChannel("coc-pyright-linting");
    this.diagnosticCollection = import_coc19.languages.createDiagnosticCollection("python");
  ...
  async lintOpenPythonFiles() {
    ...
    const promises = import_coc19.workspace.textDocuments.map(async (document) => this.lintDocument(document));
  ...
  async lintDocument(document) {
    ...
    const fsPath = import_coc19.Uri.parse(document.uri).fsPath;
    if (this.pendingLintings.has(fsPath)) {
      this.pendingLintings.get(fsPath).cancel();
      this.pendingLintings.delete(fsPath);
    }
    const cancelToken = new import_coc19.CancellationTokenSource();
    ...
    const activeLinters = await this.getActiveLinters(import_coc19.Uri.parse(document.uri));
  ...
  createDiagnostics(message, _document) {
    const position = import_coc19.Position.create(message.line - 1, message.column);
    const range = import_coc19.Range.create(position, position);
    const severity = lintSeverityToVSSeverity.get(message.severity);
    const diagnostic = import_coc19.Diagnostic.create(range, message.message, severity);
  ...
  shouldLintDocument(document) {
    ...
    const relativeFileName = import_path4.default.relative(import_coc19.workspace.root, import_coc19.Uri.parse(document.uri).fsPath);
    const ignoreMinmatches = settings.linting.ignorePatterns.map((pattern) => new import_minimatch.Minimatch(pattern, {dot: true}));
    if (ignoreMinmatches.some((matcher) => matcher.match(import_coc19.Uri.parse(document.uri).fsPath) || matcher.match(relativeFileName))) {
      this.outputChannel.appendLine(`${"#".repeat(5)} linting is ignored by python.linting.ignorePatterns`);
      return false;
    }
    const u = import_coc19.Uri.parse(document.uri);
...
var LinterProvider = class {
  constructor(context) {
    ...
    import_coc20.workspace.onDidOpenTextDocument((e) => this.onDocumentOpened(e), this.context.subscriptions);
    import_coc20.workspace.onDidCloseTextDocument((e) => this.onDocumentClosed(e), this.context.subscriptions);
    import_coc20.workspace.onDidSaveTextDocument((e) => this.onDocumentSaved(e), this.context.subscriptions);
    const disposable = import_coc20.workspace.onDidChangeConfiguration(this.lintSettingsChangedHandler.bind(this));
    this.disposables.push(disposable);
    this.disposables.push(import_coc20.commands.registerCommand("python.runLinting", this.runLinting.bind(this)));
  ...
  lintSettingsChangedHandler(e) {
    import_coc20.workspace.textDocuments.forEach((document) => {
  ...
  onDocumentClosed(document) {
    if (!document || !import_coc20.Uri.parse(document.uri).fsPath || !document.uri) {
      return;
    }
...
var RefactorProxy = class {
  ...
  async extractVariable(document, name, filePath, range) {
    const options = await import_coc21.workspace.getFormatOptions();
    const command = {
...
async function checkDocument(doc) {
  ...
  if (modified != 0) {
    import_coc22.window.showMessage("Buffer not saved, please save the buffer first!", "warning");
    return false;
  }
  return true;
}
function validateDocumentForRefactor(doc) {
  ...
  return new Promise((resolve, reject) => {
    import_coc22.workspace.nvim.command("write").then(() => {
      return resolve();
    }, reject);
  });
}
async function extractVariable(extensionDir, document, range, outputChannel) {
  ...
  if (!rope) {
    import_coc22.window.showMessage(`Module rope not installed`, "warning");
    return;
  }
  const workspaceFolder = import_coc22.workspace.getWorkspaceFolder(doc.uri);
  const workspaceRoot = workspaceFolder ? import_coc22.Uri.parse(workspaceFolder.uri).fsPath : import_coc22.workspace.cwd;
  const pythonSettings = PythonSettings.getInstance();
  return validateDocumentForRefactor(doc).then(() => {
    ...
    const rename = proxy.extractVariable(doc.textDocument, newName, import_coc22.Uri.parse(doc.uri).fsPath, range).then((response) => {
...
async function extractMethod(extensionDir, document, range, outputChannel) {
  ...
  return validateDocumentForRefactor(doc).then(() => {
    ...
    const proxy = new RefactorProxy(extensionDir, pythonSettings, workspaceRoot);
    const rename = proxy.extractMethod(doc.textDocument, newName, import_coc22.Uri.parse(doc.uri).fsPath, range).then((response) => {
      return response.results[0].diff;
    });
    return extractName(doc, newName, rename, outputChannel);
  });
  ...
  try {
    ...
    if (changeStartsAtLine >= 0) {
      ...
      for (let lineNumber = changeStartsAtLine; lineNumber < textEditor.lineCount; lineNumber += 1) {
        ...
        if (indexOfWord >= 0) {
          newWordPosition = import_coc22.Position.create(lineNumber, indexOfWord);
          break;
        }
      }
      return import_coc22.workspace.jumpTo(textEditor.uri, newWordPosition).then(() => {
        return newWordPosition;
      });
    }
    const newWordPosition_1 = null;
    if (newWordPosition_1) {
      return import_coc22.workspace.nvim.command("wa").then(() => {
        return import_coc22.commands.executeCommand("editor.action.rename", textEditor.uri, newWordPosition_1);
      });
    }
  } catch (error) {
...
${errorMessage}`);
    import_coc22.window.showMessage(`Cannot perform refactoring using selected element(s).`, "error");
...
async function activate(context) {
  const state = import_coc23.extensions.getExtensionState("coc-python");
  if (state.toString() === "activated") {
    import_coc23.window.showMessage(`coc-python is installed and activated, coc-pyright will be disabled`, "warning");
    return;
  }
  if (import_semver2.lt(process.versions.node, "12.0.0")) {
    import_coc23.window.showMessage(`Pyright needs Node.js v12+ to work, your Node.js is ${process.version}.`, "error");
    return;
  }
  const module2 = context.asAbsolutePath("node_modules/pyright/langserver.index.js");
  if (!import_fs.existsSync(module2)) {
    import_coc23.window.showMessage(`Pyright file doesn't exist, please reinstall coc-pyright`, "error");
    return;
  }
  const serverOptions = {
    module: module2,
    transport: import_coc23.TransportKind.ipc
  };
  const outputChannel = import_coc23.window.createOutputChannel("Pyright");
  const clientOptions = {
    documentSelector,
    synchronize: {
    ...
    middleware: {
      provideDefinition: async (document, position, token, next) => {
        ...
        if (import_coc23.Location.is(locations))
          return locations;
        let pyiFirst = false;
        if (Array.isArray(locations) && locations.length > 1) {
          const first = locations[0];
          const uri = import_coc23.Location.is(first) ? first.uri : first.targetUri;
  ...
  const client = new import_coc23.LanguageClient("pyright", "Pyright Server", serverOptions, clientOptions);
  client.registerFeature(new PyrightExtensionFeature());
  context.subscriptions.push(import_coc23.services.registLanguageClient(client));
  const formatProvider = new PythonFormattingEditProvider();
  context.subscriptions.push(import_coc23.languages.registerDocumentFormatProvider(documentSelector, formatProvider));
  context.subscriptions.push(import_coc23.languages.registerDocumentRangeFormatProvider(documentSelector, formatProvider));
  context.subscriptions.push(new LinterProvider(context));
  const codeActionProvider = new PythonCodeActionProvider();
  context.subscriptions.push(import_coc23.languages.registerCodeActionProvider(documentSelector, codeActionProvider, "Pyright"));
  ...
  textEditorCommands.forEach((commandName) => {
    context.subscriptions.push(import_coc23.commands.registerCommand(commandName, async (offset) => {
      ...
      await import_coc23.workspace.applyEdit(wsEdit);
    }));
  });
  const genericCommands = ["pyright.createtypestub"];
  genericCommands.forEach((command) => {
    context.subscriptions.push(import_coc23.commands.registerCommand(command, async (...args) => {
      const root = import_coc23.workspace.root;
      const module3 = args.pop();
      if (!module3) {
        import_coc23.window.showMessage(`Module name is missing`, "warning");
        return;
      }
      const cmd = {
        command,
  ...
  let disposable = import_coc23.commands.registerCommand("python.refactorExtractVariable", async (document, range) => {
    await extractVariable(context.extensionPath, document, range, outputChannel).catch(() => {
    });
  }, null, true);
  context.subscriptions.push(disposable);
  disposable = import_coc23.commands.registerCommand("python.refactorExtractMethod", async (document, range) => {
    await extractMethod(context.extensionPath, document, range, outputChannel).catch(() => {
    });
  }, null, true);
  context.subscriptions.push(disposable);
  disposable = import_coc23.commands.registerCommand("python.sortImports", async () => {
